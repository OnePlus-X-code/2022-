...
//B5 62 06 00 14 00 01 00 00 00 D0 08 00 00 80 25 00 00 01 00 01 00 00 00 00 00
9A 79 波特率9600
//B5 62 06 00 14 00 01 00 00 00 D0 08 00 00 00 96 00 00 01 00 01 00 00 00 00 00
8B 54 波特率38400
//B5 62 06 00 14 00 01 00 00 00 D0 08 00 00 00 C2 01 00 01 00 01 00 00 00 00 00
B8 42 波特率115200


//Pulse Period:200ms   Pulse Length:100ms
//B5 62 06 07 14 00 40 0D 03 00 A0 86 01 00 01 B5 62 06 07 14 00 40 0D 03 00 A0
86 01 00 01
...
u8 Period_Out_H[] = { 0xB5, 0x62, 0x06, 0x08, 0x06, 0x00, 0x64,
                      0x00, 0x01, 0x00, 0x01, 0x00, 0x7A, 0x12 };    //

u8 Period_Out_L[] = { 0xB5, 0x62, 0x06, 0x08, 0x06, 0x00, 0xC8,
                      0x00, 0x01, 0x00, 0x01, 0x00, 0xDE, 0x6A };    //

// B5 62 06 00 14 00 01 00 00 00 D0 08 00 00 00 96 00 00 01 00 01 00 00 00 00 00
// 8B 54
u8 Pulse_Period[] = { 0xB5, 0x62, 0x06, 0x07, 0x14, 0x00, 0x40, 0x0D, 0x03, 0x00,
                      0xA0, 0x86, 0x01, 0x00, 0x01, 0x01, 0x00, 0x00, 0x34, 0x03,
                      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xD1, 0xCA };
...
u8 Baud115200[] = { 0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00, 0x00,
                    0xD0, 0x08, 0x00, 0x00, 0x00, 0xC2, 0x01, 0x00, 0x01, 0x00,
                    0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB8, 0x42 };    //设置波特率

u8 Save_Con[] = {
    0xB5, 0x62, 0x06, 0x09, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF,
    0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x1D, 0xAB };    //保存数据

// ON:
// u8 POLLSH_SET[] ={0xB5 ,0x62 ,0x06 ,0x01 ,0x03 ,0x00 ,0x01 ,0x02 ,0x01 ,0x0E
// ,0x47}; u8 SOL_SET[]    ={0xB5 ,0x62 ,0x06 ,0x01 ,0x03 ,0x00 ,0x01 ,0x06
// ,0x01 ,0x12 ,0x4F}; u8 VELNED_SET[] ={0xB5 ,0x62 ,0x06 ,0x01 ,0x03 ,0x00
// ,0x01 ,0x12 ,0x01 ,0x1E ,0x67}; u8 TIMEUTC_SET[]={0xB5 ,0x62 ,0x06 ,0x01
// ,0x03 ,0x00 ,0x01 ,0x21 ,0x01 ,0x2D ,0x85}; u8 DOP_SET[]    ={0xB5 ,0x62
// ,0x06 ,0x01 ,0x03 ,0x00 ,0x01 ,0x04 ,0x01 ,0x10 ,0x4B};
...
// pvt
u8 PVT_SET[] = { 0xB5, 0x62, 0x06, 0x01, 0x03, 0x00,
                 0x01, 0x07, 0x01, 0x13, 0x51 };    // B5 62 06 01 03 00 01 07 01 13 51
...
void Init_GPS( )
{
    ...
    GPS_PUT_BUFF(
        Baud115200,
        sizeof( Baud115200 ) );    // 9600 改变波特率 115200
                                   //  ANO_Uart1_Put_Buf(Enter_Send,sizeof(Enter_Send));
                                   //
    MyDelayMs( 100 );
    GPS_UART_INIT( 19200 );
    ...
    GPS_PUT_BUFF(
        Baud115200,
        sizeof( Baud115200 ) );    // 19200 改变波特率 115200
                                   //    ANO_Uart1_Put_Buf(Enter_Send,sizeof(Enter_Send));
                                   //
    MyDelayMs( 100 );
    GPS_UART_INIT( 38400 );
    ...
    GPS_PUT_BUFF(
        Baud115200,
        sizeof( Baud115200 ) );    // 38400 改变波特率 115200
                                   //    ANO_Uart1_Put_Buf(Enter_Send,sizeof(Enter_Send));
...
void       UBLOX_M8_GPS_Data_Receive( u8 Data )
{
    ...
    else if ( state == 5 )
    {
        ...
        // protocol_length =
        // (protocol_length>(UBX_BUF_NUM-2))?(UBX_BUF_NUM-2):protocol_length;
    }
    else if ( state >= 6 )
    {
        ...
        else    // if((state-6) == (protocol_length+2))
        {
            ...
            if ( CK_A == ubx.pvt_buf[protocol_length] &&
                 CK_B == ubx.pvt_buf[protocol_length + 1] )
            {
                //
