...
const float fast_atan_table[257] = {
    ...
    7.794811e-01, 7.814612e-01, 7.834335e-01, 7.853983e-01, 7.853983e-01 };
...
//快速平方根算法
float my_sqrt( float number ) { return number * my_sqrt_reciprocal( number ); }
...
float my_HPF( float T, float hz, float x, float zoom, float range, float* zoom_adj )
{
    ...
    //     *zoom_adj += ( 1 / ( 1 + 1 / ( hz * 6.28f * T ) ) ) *
    //                  LIMIT( ( x - *zoom_adj ), ( -zoom ), ( zoom ) );
...
// 叉积
void vec_3_cross_product_err_sinx( float in1[3],
                                   float in2[3],
