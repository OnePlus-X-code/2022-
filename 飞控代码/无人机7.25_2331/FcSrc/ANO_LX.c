...
static inline void RC_Data_Task( float dT_s )
{
    ...
    {
        rc_in.rc_ch.st_data.ch_[ch_1_rol] = 1500;
        rc_in.rc_ch.st_data.ch_[ch_2_pit] = 1500;
        rc_in.rc_ch.st_data.ch_[ch_3_thr] = 1500;
        rc_in.rc_ch.st_data.ch_[ch_4_yaw] = 1500;
        ...
        // //用于模式0，避免模式0时失控，油门过大飞跑，给一个稍低于中位的油门
        // //这里会把实时XYZ-YAW期望速度置零
        // rt_tar.st_data.yaw_dps = 0;
...
//校准ESC成功后，记得关闭此功能，避免出现意外。
    ...
    else
    {
        ...
        if ( unlocked )
        {
            for ( u8 i = 0; i < 8; i++ ) { pwm[i] = LIMIT( pwm[i], 0, 1000 ); }
        }
        else
        {
            for ( u8 i = 0; i < 8; i++ ) { pwm[i] = 0; }
